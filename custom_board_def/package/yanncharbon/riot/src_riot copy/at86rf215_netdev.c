...
static int _send(struct ieee802154_hw *hw, struct sk_buff *skb)
{
    at86rf215_t *dev  = hw->priv;
    size_t len = 0;
...
//
    ...
    /* load packet data into FIFO */
    for (struct sk_buff *skbuff = skb; skbuff != NULL; skbuff = skbuff->next) {

        /* current packet data + FCS too long */
        if ((len + skbuff->len + IEEE802154_FCS_LEN) > AT86RF215_MAX_PKT_LENGTH) {
            DEBUG("[at86rf215] error: packet too large (%u byte) to be send\n",
                  (unsigned)len + IEEE802154_FCS_LEN);
            at86rf215_tx_abort(dev);
            return -EOVERFLOW;
        ...
        if (skbuff->len) {
            len = at86rf215_tx_load(dev, skbuff->data, skbuff->len, len);
        }
    }
...
// NOT IMPLEMENTED in Linux IEEE 802.15.4 stack (struct ieee802154_ops)
//static int _recv(netdev_t *netdev, void *buf, size_t len, void *info)
//{
//    netdev_ieee802154_t *netdev_ieee802154 = container_of(netdev, netdev_ieee802154_t, netdev);
//    at86rf215_t *dev = container_of(netdev_ieee802154, at86rf215_t, netdev);
