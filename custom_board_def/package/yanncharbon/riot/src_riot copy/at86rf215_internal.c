...
static inline uint8_t _get_reg_with_lock(at86rf215_t *dev, uint16_t r)
{
    ...
    ret = spi_write_then_read(dev->spi, &reg, sizeof(reg), &val, sizeof(val));
	if(!ret){
		printk(KERN_ERROR "Could not read bytes from register over spi (error %d)\n", ret);
	}
...
int at86rf215_hardware_reset(at86rf215_t *dev)
{
    ...
    udelay(CONFIG_AT86RF215_RESET_PULSE_WIDTH_US);
    gpio_set_value(dev->params.reset_pin, 1);
    udelay(AT86RF215_RESET_DELAY_US);
...
void at86rf215_reg_write(const at86rf215_t *dev, uint16_t reg, uint8_t value)
{
    ...
    ret = spi_write(dev->spi, &reg, sizeof(reg));
    if(!ret){
        goto err_write_byte;
    }

    ret = spi_write(dev->spi, &value, sizeof(value));
    if(!ret){
        goto err_write_byte;
    }
...
err_write_byte:
    printk(KERN_ERROR "Could not write bytes to register over spi (error %d)\n", ret);
}
...
uint8_t at86rf215_reg_read(const at86rf215_t *dev, uint16_t reg)
{
    ...
    ret = spi_write_then_read(dev->spi, &reg, sizeof(reg), &val, sizeof(val));
	if(!ret){
		printk(KERN_ERROR "Could not read value from register over spi (error %d)\n", ret);
	}
...
void at86rf215_reg_read_bytes(const at86rf215_t *dev, uint16_t reg, void *data, size_t len)
{
    ...
    ret = spi_write_then_read(dev->spi, &reg, sizeof(reg), data, len);
	if(!ret){
		printk(KERN_ERROR "Could not read bytes from register over spi (error %d)\n", ret);
	}
}
