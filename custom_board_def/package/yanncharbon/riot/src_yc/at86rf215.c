...
static irqreturn_t at86rf215_isr(int irq, void *data)
{
	...
	printk(KERN_DEBUG "at86rf215_isr\n");
...
static int at86rf215_probe(struct spi_device *spi)
{
	...
	pr_info("[Probing]: SPI max freq = %u\n", spi->max_speed_hz);
	pr_info("[Probing]: Sspi->of_node.name= %s , full_name= %s\n", spi->dev.of_node->name, spi->dev.of_node->full_name);
	pr_info("[Probing]: Calling driver is spi->dev.driver->name= %s\n", spi->dev.driver->name);
